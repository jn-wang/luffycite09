...
class UserToken(models.Model):
    ...
    class Meta:
        verbose_name_plural = "21.认证token表"
# ######################## 课程相关 ########################
...
class Coupon(models.Model):
    ...
    coupon_type = models.SmallIntegerField(choices=coupon_type_choices,default=0,verbose_name='卷类型')
    money_equivalent_value = models.IntegerField(verbose_name="等值货币")
    off_percent = models.PositiveSmallIntegerField('折扣百分比',help_text="只针对折扣卷，例7.9折，写79",blank=True,null=True)
    minimum_consume = models.PositiveIntegerField("最低消费",default=0,help_text="仅在减慢卷时填写此字段")

    content_type = models.ForeignKey(ContentType,blank=True,null=True)
    object_id = models.PositiveIntegerField("绑定课程",blank=True,null=True,help_text="可以将优惠卷与课程绑定")
    content_object = GenericForeignKey('content_type','object_id')

    quantity = models.PositiveIntegerField('数量（张）',default=1)
    open_date = models.DateField("优惠卷领取开始时间")
    close_date = models.DateField("优惠卷领取结束时间")
    valid_begin_date = models.DateField(verbose_name="有效期开始时间",blank=True,null=True)
    valid_end_date = models.DateField(verbose_name="有效期结束时间",blank=True,null=True)
    coupon_valid_days = models.PositiveIntegerField(verbose_name='优惠卷有效期（天）',blank=True,null=True,help_text="卷被领取时算起")

    data = models.DateTimeField(auto_now_add=True)

    class Meta:
        verbose_name_plural = "31.优惠卷生成记录"
        ...
        if not self.coupon_valid_days or (self.valid_begin_date and self.valid_end_date):
            if self.valid_begin_date and self.valid_end_date:
                if self.valid_end_date <= self.valid_begin_date:
                    raise ValueError("valid_end_data 有效期结束日期必须晚于valid_begin_data")
                if self.coupon_valid_days == 0:
                    raise ValueError('coupon_end_days 有效期不能为0')
            if self.close_date < self.open_date:
                raise ValueError("close_data 优惠卷领取结束时间必须晚于open_data优惠卷领取时间")
...
class CouponRecord(models.Model):
    ...
    used_time = models.DateTimeField(blank=True,null=True,verbose_name="优惠卷使用时间")
    # order = models.ForeignKey("Order",blank=True,null=True,verbose_name="关联订单")

    class Meta:
        verbose_name_plural = "32. 用户优惠卷"
